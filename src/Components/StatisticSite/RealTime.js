      ...
      <div>
        ...
        <Modal show={this.state.open} onHide={this.handleCloseModal} size="xl">
          <Modal.Header closeButton>
            <Modal.Title className={style.previewTitle}>
              {this.state.modalTitle}
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <iframe
              srcDoc={this.state.modalContent}
              className={style.previewContant}
            ></iframe>
          </Modal.Body>
          <Modal.Footer>
...
class RealTimeTable extends React.Component {
  ...
  handleRow = row => {
    ...
    let previewBtn = (
      <Button
        name={`${row.id}\0${row.secure === 1 ? "https://" : "http://"}${
          row.host
        }${row.uri}`}
        variant="success"
        size="sm"
