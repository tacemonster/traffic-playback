...
class RealTimeMonitor extends React.Component {
  ...
  getRealTimeData = () => {
    ...
    fetch(
      "http://ec2-54-152-230-158.compute-1.amazonaws.com:8000/api/play/realtime?limit=50"
    )
      .then(res => {
        if (res.json) {
...
class RealTimeTable extends React.Component {
  ...
  handleRow = row => {
    ...
    let date = `${rowTime.getMonth() +
      1}/${rowTime.getDate()} ${rowTime.getHours()}:${rowTime.getMinutes()}:${rowTime.getSeconds()}`;
  ...
  render() {
    ...
    return (
      <div>
        <Row>
          <Col xs={12} sm={6} lg={3}>
            <label htmlFor="columns-dropdown" className={style.optionLabel}>
              Display Columns:
            </label>
            <Dropdown>
              <Dropdown.Toggle
          ...
          <Col xs={12} sm={6} lg={3}>
            <label htmlFor="update-time" className={style.optionLabel}>
              Update By:
            </label>
            <Form.Control
              as="select"
          ...
          <Col xs={12} sm={6} lg={3}>
            ...
            <div style={{ textAlign: "center" }}>
              {!this.state.never ? (
                <span>
                  Will Refresh in <span>{this.state.remainTime}</span>s
                </span>
              ) : (
